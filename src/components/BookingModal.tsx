...
const BookingModal = ({ open, onOpenChange, preselectedBarber }: BookingModalProps) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'appointments' }, handleRealtimeAppointmentChange)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'appointments' }, handleRealtimeAppointmentChange)
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'appointments' }, handleRealtimeAppointmentChange)
      .subscribe();
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent onOpenAutoFocus={(e) => e.preventDefault()} className="bg-[#050505] border-[#c9a84c]/15 text-foreground w-full sm:w-[95vw] max-w-md max-h-[100dvh] sm:max-h-[85dvh] overflow-y-auto p-4 sm:p-6" style={{ borderRadius: 2 }}>
          <DialogHeader>
          ...
          {success ? (
            <div className="py-8 space-y-6 relative">
              {/* Giant faded checkmark background */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.04]">
                <Check size={280} strokeWidth={0.5} className="text-[#c9a84c]" />
              </div>
              
              <div className="relative z-10">
                <h2 className="font-serif text-3xl italic text-[#c9a84c] mb-2">
                  Até já, {clientName}.
                </h2>
                <p className="font-cormorant text-lg italic text-foreground/40">A sua reserva está confirmada.</p>
              </div>

              <div className="relative z-10 space-y-3" style={{ borderTop: '0.5px solid rgba(201,168,76,0.15)', paddingTop: 20 }}>
                <div className="flex justify-between py-2" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}>
                  <span className="font-sans text-[10px] font-light uppercase tracking-[0.2em] text-foreground/30">Barbeiro</span>
                  <span className="font-sans text-sm text-foreground/80">{selectedBarberName}</span>
                </div>
                <div className="flex justify-between py-2" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}>
                  <span className="font-sans text-[10px] font-light uppercase tracking-[0.2em] text-foreground/30">Serviço</span>
                  <span className="font-sans text-sm text-foreground/80">{selectedServiceObj?.name}</span>
                </div>
                <div className="flex justify-between py-2" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}>
                  <span className="font-sans text-[10px] font-light uppercase tracking-[0.2em] text-foreground/30">Data</span>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4].map(s => (
                  <div key={s} className={cn("h-[1px] flex-1 transition-colors", step >= s ? "bg-[#c9a84c]" : "bg-foreground/5")} />
                ))}
              </div>
              ...
              {step === 1 && (() => {
                ...
                return (
                <div>
                  <p className="font-sans text-[10px] font-light uppercase tracking-[0.3em] text-foreground/30 flex items-center gap-2 mb-6"><User size={12} /> {t("booking.chooseBarber")}</p>
                    ...
                    {barbers.map((b, index) => {
                      ...
                      return (
                        <button
                          ...
                          onMouseEnter={e => { if (!isSelected) (e.currentTarget as HTMLButtonElement).style.paddingLeft = "8px"; }}
                          onMouseLeave={e => { if (!isSelected) (e.currentTarget as HTMLButtonElement).style.paddingLeft = "0"; }}
                        >
                          {/* Letter */}
                          <div style={{
                            flexShrink: 0,
              ...
              {step === 2 && (
                <div className="space-y-3">
                      ...
                      return (
                        ...
                        >
                          <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
                            <span style={{ fontSize: 10, fontFamily: "'Inter', sans-serif", fontWeight: 200, letterSpacing: "1.5px", color: "rgba(201,168,76,0.25)" }}>
                            ...
                            <div style={{ textAlign: "left" }}>
                              ...
                              <span style={{ fontSize: 10, color: "rgba(255,255,255,0.18)", fontFamily: "'Inter', sans-serif", fontWeight: 200, letterSpacing: "1px", display: "block", marginTop: 3 }}>
                  ...
                  <button onClick={() => setStep(1)} className="font-sans text-[10px] font-light uppercase tracking-[0.2em] text-foreground/20 hover:text-foreground/40 transition-colors">{t("booking.back")}</button>
                </div>
              )}

              {step === 3 && (
                <div className="space-y-4">
                  <p className="font-sans text-[10px] font-light uppercase tracking-[0.3em] text-foreground/30 flex items-center gap-2"><CalendarIcon size={12} /> {t("booking.chooseDateTime")}</p>

                  {/* Calendar */}
                  <div style={{ border: '0.5px solid rgba(201,168,76,0.1)', borderRadius: 0, background: 'rgba(201,168,76,0.02)' }}>
                    <Calendar
                      mode="single"
                  ...
                  {selectedDate && (
                    <>
                      {getUrgencyMessage() && (
                        <div className="text-center py-2 px-3" style={{ border: '0.5px solid rgba(201,168,76,0.2)', borderRadius: 0 }}>
                            ...
                            return (
                              <button
                                ...
                                style={{
                                  ...
                                  color: booked ? 'rgba(255,255,255,0.12)' : isSelected ? '#c9a84c' : 'rgba(255,255,255,0.5)',
                  ...
                  <div className="flex gap-2 items-center">
                    <button onClick={() => setStep(2)} className="font-sans text-[10px] font-light uppercase tracking-[0.2em] text-foreground/20 hover:text-foreground/40 transition-colors">{t("booking.back")}</button>
              ...
              {step === 4 && (() => {
                ...
                <div style={{ padding: "0 0 14px" }}>
                  ...
                  <div style={{ opacity: 0, animation: "fadeUpForm 0.38s ease forwards", animationDelay: "0.05s", marginBottom: 18 }}>
                    {needsWarning && (
                      <div style={{ fontSize: 11, color: "#c9a84c", fontFamily: "'Inter', sans-serif", fontWeight: 300, marginBottom: 8, animation: prefShakeTriggered ? "prefShake 0.4s ease" : "none" }}>
                        Escolha como quer receber a confirmação
                      </div>
                    )}
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", fontFamily: "'Inter', sans-serif", fontWeight: 200, letterSpacing: "1px", textTransform: "uppercase" as const, marginBottom: 10 }}>
                        ...
                        return (
                          <button
                            ...
                            style={{
                              ...
                              border: `0.5px solid ${isActive ? "#c9a84c" : isRedState ? "rgba(201,168,76,0.5)" : "rgba(255,255,255,0.08)"}`,
                              borderRadius: 0,
                              padding: "10px 4px",
                              fontSize: 10,
                              color: isActive ? "#c9a84c" : isRedState ? "rgba(201,168,76,0.5)" : "rgba(255,255,255,0.3)",
                              fontFamily: "'Inter', sans-serif",
                  ...
                  {/* Nome */}
                  <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 14, opacity: 0, animation: "fadeUpForm 0.42s ease forwards", animationDelay: "0.12s" }}>
                    <span style={{ fontSize: 9, color: "rgba(201,168,76,0.4)", letterSpacing: 2, textTransform: "uppercase" as const, fontFamily: "'Inter', sans-serif", fontWeight: 200 }}>Nome</span>
                    <input
                      placeholder="Nome completo *"
                      value={clientName}
                      onChange={e => setClientName(e.target.value)}
                      style={{ background: "transparent", borderBottom: "0.5px solid rgba(201,168,76,0.15)", borderTop: "none", borderLeft: "none", borderRight: "none", borderRadius: 0, padding: "12px 0", fontSize: "16px", color: "#e0e0e0", outline: "none", width: "100%", fontFamily: "'Inter', sans-serif", fontWeight: 300, WebkitTextSizeAdjust: "none", touchAction: "manipulation", transition: "border-color 0.2s" }}
                  ...
                  {/* Email */}
                  {!hideEmailField && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 14, opacity: 0, animation: "fadeUpForm 0.42s ease forwards", animationDelay: "0.18s" }}>
                      <span style={{ fontSize: 9, color: "rgba(201,168,76,0.4)", letterSpacing: 2, textTransform: "uppercase" as const, fontFamily: "'Inter', sans-serif", fontWeight: 200 }}>Email</span>
                      <input
                        ...
                        style={{
                          background: "transparent", borderBottom: "0.5px solid rgba(201,168,76,0.15)", borderTop: "none", borderLeft: "none", borderRight: "none", borderRadius: 0, padding: "12px 0", fontSize: "16px",
                          color: "#c9a84c", outline: "none", width: "100%", fontFamily: "'Inter', sans-serif", fontWeight: 300,
                  ...
                  {/* Telefone */}
                  {!hidePhoneField && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 18, opacity: 0, animation: "fadeUpForm 0.42s ease forwards", animationDelay: "0.21s", ...(phoneDisabled ? { opacity: 0.4 } : {}) }}>
                      <span style={{ fontSize: 9, color: "rgba(201,168,76,0.4)", letterSpacing: 2, textTransform: "uppercase" as const, fontFamily: "'Inter', sans-serif", fontWeight: 200 }}>Telefone</span>
                      ...
                      >
                        ...
                        <input
                          ...
                          style={{ flex: 1, background: "transparent", border: "none", outline: "none", padding: "12px 10px", fontSize: "16px", color: "#e0e0e0", fontFamily: "'Inter', sans-serif", fontWeight: 300, WebkitTextSizeAdjust: "none", touchAction: "manipulation", cursor: phoneDisabled ? "not-allowed" : "text" }}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{
                    ...
                    borderTop: "0.5px solid rgba(201,168,76,0.1)", paddingTop: 18, marginBottom: 18,
                  }}>
                    <div style={{ fontSize: 9, color: "rgba(201,168,76,0.35)", letterSpacing: 2, textTransform: "uppercase" as const, fontFamily: "'Inter', sans-serif", fontWeight: 200, marginBottom: 12 }}>
                      Resumo
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
                      <div>
                        <div style={{ fontSize: 14, color: "#e0e0e0", fontFamily: "'Playfair Display', serif", fontWeight: 600, marginBottom: 4 }}>
                          {selectedServiceObj?.name || ""} <span style={{ color: "rgba(255,255,255,0.15)" }}>·</span> {selectedBarberName || ""}
                        </div>
                        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", fontFamily: "'Inter', sans-serif", fontWeight: 200 }}>
                          {selectedDate ? format(selectedDate, "dd/MM/yyyy") : ""} às {selectedTime}
                        </div>
                      </div>
                      <div style={{ fontSize: 28, fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", color: "#c9a84c" }}>
                        €{Number(selectedServiceObj?.price || 0).toFixed(0)}
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      width: "100%", fontSize: 11, fontWeight: 500, color: "#050505", fontFamily: "'Inter', sans-serif", letterSpacing: "3px", textTransform: "uppercase" as const,
                  ...
                  {/* Back button */}
                  <div onClick={() => setStep(3)} style={{ textAlign: "center", padding: "12px 0 2px", fontSize: 10, color: "rgba(255,255,255,0.15)", cursor: "pointer", fontFamily: "'Inter', sans-serif", fontWeight: 200, letterSpacing: "1px" }}>
                    ← Voltar
                  </div>
                </div>
                );
